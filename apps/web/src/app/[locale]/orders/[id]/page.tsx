...
import { ArrowLeft, Package, Truck, CheckCircle, XCircle, Receipt } from 'lucide-react';
...
export default function OrderDetailPage() {
  ...
  const handleDownload = () => {
    downloadReceipt('order-receipt-content', `invoice_${order.id}`);
  };
    ...
    <>
      ...
      <main className="min-h-screen py-12 bg-secondary/10">
        <div className="container mx-auto px-4 max-w-3xl space-y-6">
          <Link
            href={`/${locale}/orders`}
            className="inline-flex items-center gap-1.5 text-xs font-bold text-muted-foreground hover:text-primary transition-colors uppercase tracking-wider no-print"
          >
            <ArrowLeft size={14} />
            {t('back_to_orders')}
          </Link>
          ...
          <div className="flex gap-2 mb-6 no-print">
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2 rounded-full transition text-sm font-semibold cursor-pointer ${
                activeTab === 'timeline'
                  ? 'bg-primary text-white'
                  : 'bg-secondary/40 text-foreground hover:bg-primary/20'
              }`}
            >
              <Package size={16} />
              {t('order_timeline')}
