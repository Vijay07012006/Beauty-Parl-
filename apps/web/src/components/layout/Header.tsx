    ...
    <header className="sticky top-0 z-50 glassmorphic transition-colors duration-300">
      <div className="container mx-auto px-4 h-16 flex items-center justify-between">
            ...
            <AnimatePresence>
              {isExploreOpen && (
                <motion.div
                  ...
                  exit={{ opacity: 0, y: 15 }}
                  className="absolute top-full left-1/2 -translate-x-1/2 mt-3 bg-card/95 backdrop-blur-2xl rounded-3xl shadow-2xl border border-border/50 p-6 w-screen max-w-5xl z-50 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-6"
                >
                  ...
                  {/* Column 5: Business Partners */}
                  <div className="space-y-3">
                    <p className="text-[10px] font-bold text-accent uppercase tracking-widest">Business Portal</p>
                    <div className="flex flex-col gap-1">
                      <Link href={`/${locale}/wholesale/login`} onClick={() => setIsExploreOpen(false)} className="block p-2 rounded-xl text-xs font-semibold hover:bg-secondary/70 transition-colors">
                        💼 B2B Wholesale
                        <span className="block text-[9px] text-muted-foreground font-normal">Order bulk presets and pricing</span>
                      </Link>
                      <Link href={`/${locale}/vendor/register`} onClick={() => setIsExploreOpen(false)} className="block p-2 rounded-xl text-xs font-semibold hover:bg-secondary/70 transition-colors">
                        🤝 Partner Registry
                        <span className="block text-[9px] text-muted-foreground font-normal">List your brand on our marketplace</span>
                      </Link>
                    </div>
                  </div>
                </motion.div>
        ...
        <div className="flex items-center gap-2 md:gap-3 lg:gap-4">
          ...
          {/* More Dropdown (Always visible at the far right end on desktop) */}
          <div
