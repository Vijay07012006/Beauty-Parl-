...
export class StripeService {
  ...
  constructor(
    ...
    if (!secretKey) {
      console.warn(
        '⚠️ [Payments] Stripe Secret Key is not defined. Stripe payments will fail.',
      );
    } else {
  ...
  async createCheckoutSession(
    ...
    if (!this.stripe) {
      throw new InternalServerErrorException(
        'Stripe service is not configured. Payments are disabled.',
      );
    }
    ...
    try {
      ...
      throw new InternalServerErrorException(
        error.message || 'Stripe checkout session generation failed.',
      );
    }
  }
  ...
  async handlePaymentSuccess(session: Stripe.Checkout.Session) {
    ...
    if (order.status === 'paid') {
      console.log(
        `[Stripe Webhook] Order #${order.id} is already paid. Skipping.`,
      );
      return;
    }
    ...
    if (session.payment_status !== 'paid') {
      console.warn(
        `[Stripe Webhook] Session for order #${order.id} has payment_status '${session.payment_status}'. Skipping.`,
      );
      return;
    }
    ...
    if (session.currency && session.currency.toLowerCase() !== 'inr') {
      console.warn(
        `[Stripe Webhook] Currency mismatch for order #${order.id}: ${session.currency}. Skipping.`,
      );
      return;
    }
    ...
    if (typeof session.amount_total !== 'number') {
      console.warn(
        `[Stripe Webhook] Missing amount_total for order #${order.id}. Skipping.`,
      );
      return;
    }
    ...
    if (!updated.affected || updated.affected === 0) {
      console.log(
        `[Stripe Webhook] Order #${order.id} already processed. Skipping.`,
      );
      return;
    }
    order.status = 'paid';
    ...
    if (email) {
      try {
        ...
        console.log(
          `📧 [Stripe Webhook] Confirmation email sent to ${email} for Order #${order.id}`,
        );
      } catch (err) {
        console.error(
          `❌ [Stripe Webhook] Failed to send confirmation email for Order #${order.id}:`,
          err,
        );
      }
    }
